...
def main() -> int:
    parser = argparse.ArgumentParser(description="Re-run packaged main causal models")
    parser.add_argument("--sample", choices=["main", "complete_case", "full"], default="main")
    args = parser.parse_args()

    run_step(
        "dml_estimation_track1_per_cell.py",
        "--sample",
        args.sample,
        "--output-dir",
        str(MODEL_ROOT / "track1_per_cell_v1"),
    )
    run_step(
        "dml_estimation_track1_ilr.py",
        "--sample",
        args.sample,
        "--output-dir",
        str(MODEL_ROOT / "track1_ilr_v1"),
    )
    run_step(
        "dml_estimation_track2.py",
        "--sample",
        args.sample,
        "--output-dir",
        str(MODEL_ROOT / "track2_v1"),
    )
    return 0
